...
const TimeBackground = ({ nowHoursValue }: BackgroundProps) => {
  ...
  const timeZone: timeZoneI = useMemo((): timeZoneI => {
    if (nowHoursValue < 3 || nowHoursValue > 20) {
      // 새벽 3시 전, 저녁 8시 이후 => 밤
      return {
    ...
    } else if (nowHoursValue > 2 && nowHoursValue < 6) {
      // 새벽 2시 이후, 새벽 6시 전 => 새벽
      return {
        loadTextureName: "night.png",
        sunLightPosition: [0, 80, -65],
    ...
    } else if (nowHoursValue > 5 && nowHoursValue < 12) {
      // 새벽 5시 이후, 오후 12시 전 => 아침
      return {
    ...
    } else if (nowHoursValue > 11 && nowHoursValue < 17) {
      // 오전 11시 이후, 오후 5시 전 => 낮
      return {
  ...
  useEffect(() => {
    ...
    directionalLight1.position.set(
      timeZone.sunLightPosition[0],
      timeZone.sunLightPosition[1],
      timeZone.sunLightPosition[2]
    );
