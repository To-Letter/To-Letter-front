...
interface props {
  onChangheAddress: (address: string) => void
  onClickOpenModal: () => void
}

function AddressModal({onChangheAddress, onClickOpenModal}: props) {
  ...
  const searchAddress = async () => {
    if(searchWord !== ''){
      try {
        ...
        const res = await axios.get(`https://business.juso.go.kr/addrlink/addrLinkApi.do?confmKey=${process.env.REACT_APP_ADDRESS_API_KEY}&currentPage=${common.currentPage+1}&countPerPage=20&keyword=${searchWord}&resultType=json`);
  ...
  const searchAddressButton = async () => {
    if(searchWord !== ''){
      try {
        ...
        const res = await axios.get(`https://business.juso.go.kr/addrlink/addrLinkApi.do?confmKey=${process.env.REACT_APP_ADDRESS_API_KEY}&currentPage=0&countPerPage=20&keyword=${searchWord}&resultType=json`);
        if (res.data.results.common.errorCode === "0") {
  ...
  return (
        <ModalContent>
          <ModalHeader>
            ...
            <OutBtn onClick={onClickOpenModal}>X</OutBtn>
          </ModalHeader>
          <ModalContents>
            <SearchBox>
